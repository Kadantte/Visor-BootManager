...
static int anim_tick(gui_state_t *state) {
    ...
    /* Advance the deadline by whole frame steps from the one we just met, so a
       frame that arrives late does not push every later frame out with it.
       Resetting to now + hold instead re-phases playback to whenever the tick
       happened to land, and since a full scene rebuild runs per animated frame,
       late ticks are routine rather than exceptional -- the drift accumulates
       into a visible lurch.  When the backlog exceeded what we are willing to
       skip we did not catch up, so drop the surplus and restart the phase from
       now; carrying it would leave every subsequent frame instantly overdue. */
    if (clamped)
        a->next_ms = now + hold;
    else
